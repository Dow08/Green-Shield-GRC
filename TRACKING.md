...
**V-03 (Majeur) — upload sans plafond de taille.** `POST /projects/{p_id}/upload` écrivait le flux reçu sur disque via `shutil.copyfileobj` sans aucune limite — un seul upload pouvait saturer le disque. Nouveau `path_safety.lire_upload_borne()` : lecture par blocs de 1 Mo, rejet `HTTPException 413` dès que le total dépasse le plafond, sans attendre la fin du flux. Plafond de 20 Mo (fichiers cibles réels : `sshd_config`/`nginx.conf`, quelques Ko). Le même helper corrige au passage **V-07** : `POST /projects/import-archive` faisait `await file.read()` sans plafond, avant même que `archive.lire_archive()` ait la moindre chance de contrôler la taille décompressée (200 Mo existants) — plafond de 220 Mo sur le flux brut.

**V-02 (Majeur) — aucune révocation de session.** Un JWT valide 24h restait utilisable jusqu'à expiration naturelle même après « déconnexion » côté client. Chaque jeton émis porte désormais un `jti` unique (`secrets.token_hex(16)`, indépendant de l'utilisateur — révoquer un jeton n'invalide QUE lui-même, pas les autres sessions actives du même compte). Nouvelle table `revoked_tokens` (SQLAlchemy, créée automatiquement comme le reste du schéma auth), `get_current_user` la consulte à chaque requête. Nouvelle route `POST /api/auth/logout` : décode le jeton en cours, l'ajoute à la liste de révocation (purge au passage les entrées déjà expirées naturellement — la table ne grossit jamais sans limite). Le frontend (`App.tsx::handleLogout`) l'appelle désormais en best-effort avant `clearSession()` : un réseau coupé ou un jeton déjà expiré ne doit jamais bloquer la déconnexion locale.

**V-04 (Mineur) — rate limiting générique sur les routes coûteuses.** Les 14 routes d'export (`exports.py`) et `/copilot/ask` ne dépendaient que du plafond partagé de 60/minute par IP (`main.py`), un seul endpoint coûteux pouvant consommer tout ce budget pour les autres. `@limiter.limit("30/minute")` sur les 14 routes d'export, `"20/minute"` sur `/copilot/ask` (la plus coûteuse, appel LLM potentiel). **Piège rencontré** : slowapi exige un paramètre `request: Request` sur la fonction décorée, et l'inspecte par position quand il n'est pas trouvé par mot-clé — l'insérer en 2ᵉ position (juste après `p_id`) décalait silencieusement tous les appels positionnels des ~90 tests existants qui appellent ces routes directement (`fn(p_id, data, current_user, db)`), sans lever d'erreur visible : le décalage était masqué par `_resolve_test_deps()`, qui re-résout `current_user`/`db` dès qu'ils ressemblent à un marqueur `Depends` non résolu, et `current_user` n'est de toute façon jamais utilisé pour du filtrage (mono-poste). Corrigé en plaçant `request: Request = None` en **dernière position** : FastAPI appelle toujours les endpoints par mot-clé (`dependant.call(**values)`), donc la position ne change rien en production, mais les appels positionnels existants des tests retombent sur les bons paramètres.

**V-05 (Mineur) — `modele` non échappé dans l'URL Gemini.** Seul point d'entrée où `modele` s'interpole dans une URL plutôt que dans un corps JSON déjà échappé par `json.dumps` (Ollama/Anthropic/OpenAI/Kimi) : `f"...models/{modele}:generateContent"` sans validation. Liste blanche stricte (`^[A-Za-z0-9._-]+$`, couvre tous les formats réels de nom de modèle Gemini) avant construction de l'URL — rejet silencieux (`None`, cohérent avec le contrat existant d'`appeler_llm`) sinon.

**V-06 (Mineur) — en-têtes de sécurité absents hors nginx.** CSP/X-Frame-Options/X-Content-Type-Options/Referrer-Policy ne vivaient que dans `web/nginx.conf` (déploiement Docker) — absents en mode `uvicorn` direct et dans l'exécutable de bureau (PyInstaller, sans reverse proxy devant), les deux modes de déploiement réels hors Docker. Nouveau middleware `ajouter_en_tetes_securite` dans `main.py`, mêmes valeurs exactes que `nginx.conf`.

**21 tests ajoutés** : `test_path_safety.py` (4, `lire_upload_borne` isolé), `test_projects_security.py` (1, V-03), `test_archive_routes.py` (1, V-07), `test_auth.py` (5, nouveau fichier — flux HTTP complet register/login/logout/revocation, y compris qu'un jeton révoqué n'affecte pas les autres sessions du compte), `test_rate_limiting.py` (3, nouveau fichier — plafonds réellement déclenchés via `TestClient`, plafonds indépendants entre routes), `test_ai_gateway.py` (4, V-05), `test_security_headers.py` (3, nouveau fichier). Vérifié aussi en conditions réelles (serveur `uvicorn` lancé en direct, hors mocks) : cycle register/login/logout/401-après-logout, et 31 requêtes consécutives sur une route d'export confirmant 30×404 puis 429. **748 tests backend, 183 tests frontend.**

**Faux positif CI rencontré après le push** : le job « Secret Scanning » (TruffleHog, `--only-verified`) a signalé et *vérifié* comme clé Lob valide le nom d'une fonction de test ajoutée dans `test_rate_limiting.py` — coïncidence de forme pure : le détecteur Lob matche `\b(test|live)_[a-zA-Z0-9_]{35}\b`, et cet identifiant Python faisait par hasard exactement `test_` + 35 caractères `[a-zA-Z0-9_]`. Aucune clé n'a jamais existé dans le dépôt ; confirmé en récupérant l'annotation du check-run (fichier + ligne exacts) via l'API GitHub (authentifiée avec les identifiants déjà stockés par le credential helper git, `gh` CLI absent de cet environnement) plutôt que de deviner. Corrigé en renommant l'identifiant pour changer sa longueur (un second nom préexistant de forme identique renommé par précaution).

**Piège pour la prochaine fois, à deux niveaux** : (1) renommer une ligne ne suffit pas forcément à faire repasser le job au vert *sur ce commit précis* — TruffleHog scanne le texte du diff `avant → après` du push, qui contient toujours l'ancien nom en ligne supprimée ; il faut un push *suivant*, dont le diff ne touche plus cette zone, pour obtenir un run réellement propre — le commit intermédiaire reste rouge dans son propre historique, ce qui est normal et sans conséquence. (2) **Ne jamais reproduire le nom exact déclencheur dans une note qui documente l'incident** (première version de ce paragraphe : re-déclenché sur cette ligne elle-même) — décrire le motif plutôt que de citer la chaîne complète.
...
Les autres constats de l'audit (révocation de session absente, upload sans plafond de taille, rate limiting générique sur les exports/Copilote, en-têtes de sécurité absents hors nginx, `modele` non échappé dans l'URL Gemini) ont été traités dans la foulée — voir l'entrée suivante (V-02 à V-07).
...
**Validation Pydantic + fuites `str(exc)`** (point mineur différé depuis l'audit du 31/07/2026) : `import_framework` utilise enfin `ImportFrameworkRequest` — le schéma existait déjà, complet avec un `ExigenceImport` imbriqué, mais n'avait jamais été branché sur la route (le corps restait un `dict` brut). `update_project` valide désormais l'identité de la mission (`id`/`name` non vides, `id` du corps qui doit correspondre à celui de l'URL) via un nouveau `UpdateProjectRequest` **permissif** (`model_config = ConfigDict(extra="allow")`) : la forme interne (`steps`/`socle`/`grc`/`consulting`) reste volontairement non contrainte puisqu'elle évolue via la chaîne de migration, pas via ce schéma — sans ce garde-fou minimal, un corps de requête vide ou tronqué écrasait silencieusement `project.json` sur disque. En creusant, la recherche des fuites `str(exc)`/`f"...{exc}"` a trouvé **9 sites**, pas les 2 initialement cités : `main.py` (audit AuditCraft), `crud.py` (import d'archive, workflow de référentiel — celui-ci exposait carrément un chemin disque absolu dans le message), `exports.py` et `snapshots_routes.py` (chacun portait une copie identique de `_lire_state_pour_docx` avec la même fuite), `collecte_technique.py`. Chaque site logue désormais l'exception complète côté serveur (`logging.exception`) et renvoie un message générique au client. **5 tests ajoutés** (`test_projects_security.py`).

**Racine du dépôt rangée** — plusieurs incohérences de nommage et de la clutter, gênantes pour un dépôt qui sert de vitrine :
- `AuditCraft-GRC/` à la racine (uniquement du `__pycache__`, non suivi par git — reliquat d'avant le déplacement du module dans `api/modules/auditcraft_grc/`) : supprimé.
- Deux worktrees git abandonnés sous `.claude/worktrees/` (`recursing-diffie-1a4811`, `vigorous-williams-98d44f`), tous deux en retard sur `main`, dont un portait une modification non commitée de `report_builder.py` — vérifiée : strictement identique à `_protection_donnees_md` déjà fusionnée dans `main` le 31/07/2026. Retirés proprement via `git worktree remove`, branches locales orphelines supprimées.
- `LOGO.png` (2 Mo) à la racine, jamais référencé nulle part dans le code — le logo réellement utilisé est `web/public/logo.png` (103 Ko, optimisé) et sa version embarquée en base64 dans `charte.py`. Retiré du suivi git.
- `ia_archives/` (scripts `fix_*`/`update_tests*` d'anciennes sessions, et un dump brut `transcript_user.txt` d'un prompt utilisateur) : n'était déjà plus référencé nulle part, retiré entièrement — l'historique git le conserve si nécessaire.
- **`lab_target/` et `target_lab/` fusionnés** — deux répertoires distincts au nom quasi interverti (l'un portait le `Dockerfile` du service `target_lab`, l'autre les configurations vulnérables `sshd_config`/`nginx.conf` montées à la fois dans ce service et dans `api`), source de confusion évidente. Les fichiers de `lab_target/` déplacés vers `target_lab/config/` (`git mv`, historique préservé), `docker-compose.yml` et `api/main.py::TARGET_DIR` mis à jour en conséquence, scan AuditCraft-GRC revérifié en direct après la bascule (mêmes résultats qu'avant : score 25, 8 contrôles évalués). Au passage, mojibake corrigé dans l'en-tête de `docker-compose.yml` (encodage UTF-8/Latin-1 mal interprété).

**README enrichi et corrigé** — plusieurs pans de l'application livrés cette session n'y figuraient pas du tout : la passerelle IA **à cinq fournisseurs** (Ollama/Gemini/Anthropic/OpenAI/Kimi, le README ne décrivait encore que Gemini seul) avec la recommandation de modèle par détection matérielle, la double lecture NIST CSF (roue de couverture **et** radar de maturité, jamais confondus), le registre des demandes de preuves, et la portée exacte de la dictée vocale (liste d'exclusion nominative, pas un simple interrupteur). Coquille corrigée dans le paragraphe de genèse (« d'oubliédes information importante » → « d'oublier une information importante », introduite par un commit externe le 06/08/2026). Compteurs de tests actualisés (705/183), badge CI ajouté, mention de l'exécutable Windows ajoutée à la section française (n'existait qu'en anglais). Sections FR et EN tenues en parité.

Vérifié : `py -3 -m pytest api/tests -q` (705 passed), `docker compose config` (syntaxe valide), scan AuditCraft-GRC relancé en direct après la fusion des répertoires cibles.
