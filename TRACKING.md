...
**Faux positif CI rencontré après le push** : le job « Secret Scanning » (TruffleHog, `--only-verified`) a signalé et *vérifié* comme clé Lob valide le nom du test `test_route_d_export_est_limitee_en_debit` — coïncidence de forme pure : le détecteur Lob matche `\b(test|live)_[a-zA-Z0-9_]{35}\b`, et cet identifiant Python fait exactement `test_` + 35 caractères `[a-zA-Z0-9_]`. Aucune clé n'a jamais existé dans le dépôt ; confirmé en récupérant l'annotation du check-run (fichier + ligne exacts) via l'API GitHub (authentifiée avec les identifiants déjà stockés par le credential helper git, `gh` CLI absent de cet environnement) plutôt que de deviner. Corrigé en renommant l'identifiant (un second nom préexistant de forme identique renommé par précaution). **Piège pour la prochaine fois** : renommer une ligne ne suffit pas forcément à faire repasser le job au vert *sur ce commit précis* — TruffleHog scanne le texte du diff `avant → après` du push, qui contient toujours l'ancien nom en ligne supprimée ; il faut un push *suivant*, dont le diff ne touche plus cette zone, pour obtenir un run réellement propre. Le tip actuel du dépôt est vert ; le commit intermédiaire reste rouge dans son propre historique, ce qui est normal et sans conséquence.
