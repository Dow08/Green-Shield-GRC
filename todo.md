...
- [ ] **Non traité (Mineur, différé)** : validation Pydantic sur `update_project`/`import_framework` (corps `dict` brut), messages d'erreur `str(exc)` renvoyés au client par endroits, en-têtes de sécurité absents si FastAPI est joint hors du proxy nginx.
...
- **Pas encore commité** (règle CLAUDE.md : jamais sans demande explicite) — `git status` avant tout commit, plusieurs fichiers de correctifs (`fix_*.py`, `append_routes.py`, `out.txt`, `old_projects.txt`) traînent non suivis à la racine, à trier avant un `git add`.
