...
export function PhaseResilience({ activeProject, updateStepData, handleSaveProject }: Props) {
  ...
  return (
              <div className="flex flex-col gap-4">
                ...
                {/* MANUAL GRC CHECKLIST — regroupée par référentiel actif
                    (une mission peut en porter plusieurs depuis le 31/07/2026) */}
                {activeProject.type === "grc" && (() => {
                  const tousControles = activeProject.steps.evaluation?.manual_controls || [];
                  // Les missions migrées avant le multi-référentiel n'ont pas
                  // forcément de `referentiel_id` sur chaque contrôle : on
                  // retombe sur le référentiel pivot de la mission plutôt que
                  // de perdre le groupement.
                  const referentiels = Array.from(new Set(
