...
# dans snapshots_routes.py, en triple avec crud.py — supprimée ici, la
# version authentifiée vit uniquement dans `crud.py`.
...
# double de `crud.py` (dupliqué une 3e fois dans snapshots_routes.py) — trois
...
async def export_project_html(p_id: str, auditeur: str = "", cabinet: str = "", logo: str = "", current_user: User = Depends(get_current_user), request: Request = None) -> Response:
    ...
    nom, html = await asyncio.to_thread(report_html.build_report, state, p_id, auditeur, cabinet, logo)
...
def export_project_pdf(p_id: str, doc_type: str, auditeur: str = "", cabinet: str = "", current_user: User = Depends(get_current_user), request: Request = None) -> HTMLResponse:
    ...
    # Le navigateur imprime le titre de l'onglet en en-tête de page : on lui
    # donne le nom du livrable, pas son nom de fichier.
    titre_page = Path(title).stem.replace("_", " ")

    # Wrap in printable HTML layout
    # Attention aux accolades : dans cette f-string, celles du CSS doivent être
    # doublées, celles des variables non. Elles l'étaient toutes — le document
    # sortait avec le texte « {html_body} » à la place de son contenu, et
    # « {title} » dans l'onglet (corrigé le 01/08/2026).
...
<head>
    ...
    <style>
        ...
        h1, h2, h3 {{ color: #111; }}
        h1 {{ border-bottom: 2px solid #2ee6a0; padding-bottom: 10px; }}
...
<body onload="window.print()">
    ...
    audit_log.record("project.export", target=p_id, detail=f"format=pdf type={doc_type}")
